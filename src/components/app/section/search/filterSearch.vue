<template>
	<div class="filterSearch">
		<div class="type">
			...
			<div class="searchTypeResult">
				<div v-if="typeModel == 'Search By Date'">
					<v-container text-xs-center grid-list-xs class="pl-1 pr-1">
						<v-layout row wrap> 
							<v-flex xs6 md6>
							<v-dialog
						        ref="dialog"
				...
				</div> <!-- END SEARCH BY DATE HEYYYYYYYYYYO -->

				<div v-if="typeModel == 'Search By Country'">
					<v-select
					...
					<v-btn block @click="search_country">
						Search <v-icon>search</v-icon>
					</v-btn>
<postCard v-for="(post,index) in posts" :key="index" :url="url" :post="post"></postCard>
					 <div v-if="posts.length < 1" style="font-size: 22px; font-weight: 700; text-align: center; padding: 10px;">
				...
				</div> <!-- End Search By Country -->

				<div v-if="typeModel == 'Search By Catgoray'">
					<v-select
					v-model="catgModel"
					:items="subCatg"
					solo
					label="Select Catgoray"></v-select>
					<v-text-field
					v-model="search"
					label="Search Something ..."
					solo></v-text-field>

					<v-btn block @click="search_catgoray">
						Search <v-icon>search</v-icon>
					</v-btn>
		<postCard v-for="(post,index) in posts" :key="index" :url="url" :post="post"></postCard>
					 <div v-if="posts.length < 1" style="font-size: 22px; font-weight: 700; text-align: center; padding: 10px;">
...
<script>
	...
	export default {
		...
		data() {
			return {
				type:['Search By Date','Search By Country','Search By Catgoray'],
				typeModel:'',
				date: null,
				date2:null,
			      modal: false,
		...
		methods:{
			search_date() {
				const vm = this;
				axios.post(vm.url + 'api/search/filter/date',{date_from:vm.date,date_to:vm.date2}).then(response => {
			...
			search_country() {
				const vm = this;
				axios.post(vm.url + 'api/search/filter/country',{country:vm.countryModel,search:vm.search}).then(response => {
					vm.posts =response.data;
				}).catch(err => {
				});
			},
			search_catgoray() {
				const vm = this;
				axios.post(vm.url + 'api/search/filter/catgoray',{catgoray:vm.catgModel, search:vm.search}).then(response => {
					vm.posts = response.data;
				}).catch(err => {
				});
			},
