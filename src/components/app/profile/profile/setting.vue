<template>
  ...
  <v-btn slot="activator">
            <v-icon class="primary--text">build</v-icon> Settings 
	...
	<v-card class="setting">
		...
		<v-toolbar dark >
          ...
          <v-spacer></v-spacer>
          <v-toolbar-items>
            <v-btn dark flat @click.native="update">Save</v-btn>
          </v-toolbar-items>
        </v-toolbar>
      <div v-if="type == 'all'">
        <v-list three-line subheader>
          <v-subheader>User Controls</v-subheader>
          <v-list-tile avatar @click="type = 'update'">
            <v-list-tile-content>
              <v-list-tile-title>Profile</v-list-tile-title>
              <v-list-tile-sub-title>Update profile setting - Photo, Name , About and other</v-list-tile-sub-title>
            </v-list-tile-content>
          </v-list-tile>
          <v-list-tile avatar>
            <v-list-tile-content>
...
<script>
    ...
    watch:{
      dialog() {
        ...
        if(vm.dialog == true)
        {
           var locationStr = vm.profile.location;
           locationStr = locationStr.split(',');
            vm.profile.country = locationStr[0];
            vm.profile.city = locationStr[1];
        }else {
        }
      }
    },
    methods:{
      update() {
        const vm = this;
        axios.post(vm.url + 'api/user/update',{user:vm.user,profile:vm.profile}).then(response=> {
          vm.sn.snackbar = true;
          vm.sn.color = 'success';
          vm.sn.text = 'You Have Update You profile !'
          setTimeout(function() {
            location.reload()
          }, 2000);
        }).catch(err => {
